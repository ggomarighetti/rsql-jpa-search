...
class SpringConfigurationMetadataIT {
    ...
    void publishedJarContainsDescribedSpringConfigurationMetadata() throws Exception {
        ...
        try (ZipFile zip = new ZipFile(jar.toFile())) {
            ...
            try (InputStream input = zip.getInputStream(entry)) {
                ...
                assertProperty(properties, "search.helper.rsql.enabled", "java.lang.Boolean", "true");
                assertProperty(
                        properties,
                        "search.helper.filter.max-comparisons",
                        "java.lang.Integer",
                        null);
                assertProperty(
                        properties,
                        "search.helper.rsql.perplexhub.strict-equality",
                        "java.lang.Boolean",
                        "true");
            }
        }
    ...
    private static Path projectJar() throws Exception {
        try (Stream<Path> files = Files.list(Path.of("target"))) {
            return files.filter(path -> path.getFileName().toString().matches("search-helper-[^/]+\\.jar"))
                    .findFirst()
